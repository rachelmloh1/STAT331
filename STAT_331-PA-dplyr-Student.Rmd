...
colleges_clean <- colleges %>% 
  ...
  mutate(CONTROL = as.factor(CONTROL),REGION = as.factor(REGION)) %>%
  mutate(TUITION_DIFF = TUITIONFEE_OUT - TUITIONFEE_IN) %>%
  na.omit()
...
colleges_clean <- colleges_clean %>%
  filter(REGION == "1", quantile(ADM_RATE, 1, na.rm =TRUE, names = TRUE), 
  TUITION_DIFF == 0, SAT_AVG %% 2 != 0, CITY != "New Hampshire", CITY != "Boston", UGDS/4 > 3000*ADM_RATE, INSTNM != "Harvard University")
